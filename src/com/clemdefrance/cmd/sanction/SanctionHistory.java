...
implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        UUID viewerUUID = SanctionDB.getOfflineUUID(viewer.getName());
        if (args.length == 0) {
        ...
        if (args.length == 1 && !this.isNumber(args[0])) {
            ...
            UUID uuid = SanctionDB.getOfflineUUID(targetName);
            if (uuid == null) {
        ...
        viewer.sendMessage(ChatColor.RED + "[Sanction] Utilisation : /history [player] [page]");
        return true;
    }
}
